    ...
    return (
      <div className='min-h-screen flex items-center justify-center'>
        <div className='text-center' data-testid='statistics-error-state'>
          ...
          <p className='text-gray-600 dark:text-gray-400 mb-4' data-testid='statistics-error-message'>
            {error}
          </p>
  ...
  return (
    <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8' data-testid='statistics-page'>
      ...
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8' data-testid='statistics-header'>
        <div>
          <h1 className='text-3xl font-bold text-gray-900 dark:text-white' data-testid='statistics-title'>
            Statistics
          </h1>
          ...
          <div className='card p-6'>
            <div className='flex items-center'>
              ...
              <div className='ml-4'>
                ...
                <p className='text-2xl font-bold text-gray-900 dark:text-white'>{stats.overview.completed_todos}</p>
              </div>
            </div>
          </div>
          ...
          <div className='card p-6'>
            <div className='flex items-center'>
              ...
              <div className='ml-4'>
                ...
                <p className='text-2xl font-bold text-gray-900 dark:text-white'>{stats.overview.total_categories}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
        ...
        {todoStats && (
          <div className='space-y-6'>
            ...
            <div className='card p-6'>
              <h2 className='text-lg font-semibold text-gray-900 dark:text-white mb-4'>Todos by Priority</h2>
                ...
                <div className='space-y-3'>
                  {stats.todos_by_priority?.map(priority => (
                    <div key={priority.priority} className='flex items-center justify-between'>
                      <div className='flex items-center'>
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getPriorityColor(priority.priority)}`}>
                          {priority.priority.charAt(0).toUpperCase() + priority.priority.slice(1)}
                        </span>
                        <span className='ml-3 text-sm text-gray-600 dark:text-gray-400'>{priority.todo_count} todos</span>
                      </div>
                      <div className='text-sm font-medium text-gray-900 dark:text-white'>{priority.percentage}%</div>
                    </div>
                  ))}
                </div>
              ) : (
              ...
              {todoStats.by_category && todoStats.by_category.length > 0 ? (
                <div className='space-y-3'>
                  {todoStats.by_category.slice(0, 5).map(category => (
                    <div key={category.category_name || 'uncategorized'} className='flex items-center justify-between'>
                      <div className='flex items-center'>
                        <div className='w-3 h-3 rounded-full mr-3' style={{ backgroundColor: category.category_color || '#6b7280' }} />
                        <span className='text-sm text-gray-900 dark:text-white'>{category.category_name || 'Uncategorized'}</span>
                      </div>
                      <div className='text-sm font-medium text-gray-600 dark:text-gray-400'>{category.total_todos} todos</div>
                    </div>
                  ))}
                </div>
              ) : (
        ...
        {userStats && (
          <div className='space-y-6'>
                  ...
                  {stats.recent_activity.slice(0, 5).map((activity, index) => (
                    <div key={index} className='flex items-center justify-between'>
                      <div>
                        ...
                        <p className='text-xs text-gray-600 dark:text-gray-400'>by {activity.username}</p>
                      </div>
                      <div className='text-xs text-gray-500 dark:text-gray-500'>{formatDateTime(activity.created_at)}</div>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            {/* Top Users */}
            <div className='card p-6'>
              <h2 className='text-lg font-semibold text-gray-900 dark:text-white mb-4'>Most Active Users</h2>
                    ...
                    <div key={user.id} className='flex items-center justify-between'>
                      <div className='flex items-center'>
                        ...
                        <div>
                          ...
                          <p className='text-xs text-gray-600 dark:text-gray-400'>{user.completed_last_30_days} completed this month</p>
                        </div>
                      </div>
                      <div className='text-sm font-medium text-gray-600 dark:text-gray-400'>{user.todos_last_30_days} todos</div>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <div className='card p-6'>
              ...
              <div className='grid grid-cols-2 gap-4'>
                <div className='text-center'>
                  <p className='text-2xl font-bold text-green-600 dark:text-green-400'>{userStats.summary?.new_users_last_7_days || 0}</p>
                  <p className='text-xs text-gray-600 dark:text-gray-400'>New users (7 days)</p>
                </div>
                <div className='text-center'>
                  <p className='text-2xl font-bold text-blue-600 dark:text-blue-400'>{userStats.summary?.new_users_last_30_days || 0}</p>
              ...
              <table className='min-w-full divide-y divide-gray-200 dark:divide-gray-700'>
                ...
                <tbody className='divide-y divide-gray-200 dark:divide-gray-700'>
                  {todoStats.monthly_trends.map((trend, index) => (
                    <tr key={index}>
                      <td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white'>{formatDate(trend.month)}</td>
                      <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400'>{trend.todos_created}</td>
                      <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400'>{trend.todos_completed}</td>
                      <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400'>{trend.completion_rate}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
