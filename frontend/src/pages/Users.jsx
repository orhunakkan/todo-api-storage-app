  ...
  return (
    <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8' data-testid='users-page'>
      ...
      <div className='mb-8' data-testid='users-header'>
        <h1 className='text-3xl font-bold text-gray-900 dark:text-white' data-testid='users-title'>
          Users
        </h1>
        <p className='mt-2 text-gray-600 dark:text-gray-400' data-testid='users-subtitle'>
      ...
      {error ? (
        <div className='text-center py-8' data-testid='users-error-state'>
          ...
          <p className='text-gray-600 dark:text-gray-400 mb-4' data-testid='users-error-message'>
            {error}
          </p>
        ...
        <div className='text-center py-12' data-testid='users-no-results'>
          ...
          <p className='text-gray-600 dark:text-gray-400' data-testid='users-no-results-message'>
                    ...
                    <div className='pt-3 border-t border-gray-200 dark:border-gray-700' data-testid={`user-stats-${user.id}`}>
                      <div className='grid grid-cols-2 gap-4 text-center'>
                        <div data-testid={`user-total-todos-${user.id}`}>
                          ...
                          <p className='text-lg font-semibold text-gray-900 dark:text-white'>{user.todo_stats.total_todos}</p>
                        </div>
                        <div data-testid={`user-completed-todos-${user.id}`}>
                          <p className='text-sm text-gray-600 dark:text-gray-400'>Completed</p>
                          <p className='text-lg font-semibold text-green-600 dark:text-green-400'>{user.todo_stats.completed_todos}</p>
                        </div>
                      </div>
                      ...
                      {user.todo_stats.total_todos > 0 && (
                        <div className='mt-3' data-testid={`user-completion-rate-${user.id}`}>
                          <div className='flex justify-between text-xs text-gray-600 dark:text-gray-400 mb-1'>
                            ...
                            <span>{Math.round((user.todo_stats.completed_todos / user.todo_stats.total_todos) * 100)}%</span>
                          </div>
