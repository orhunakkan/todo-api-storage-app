        ...
        <div className='lg:col-span-1' data-testid='profile-overview'>
          <div className='card p-6'>
            <div className='text-center' data-testid='profile-avatar-section'>
              ...
              <p className='text-gray-600 dark:text-gray-400' data-testid='profile-username'>
                @{user?.username}
              </p>
            ...
            <form onSubmit={handleSubmit} className='space-y-6' data-testid='profile-form'>
              ...
              <div data-testid='profile-username-field'>
                ...
                <input
                  ...
                  onChange={handleChange}
                  disabled={!isEditing}
                  className={`input ${!isEditing ? 'bg-gray-50 dark:bg-gray-800' : errors.username ? 'border-red-500 focus:ring-red-500' : ''}`}
                  placeholder='Username'
                  data-testid='profile-username-input'
                />
                {errors.username && (
                  <p className='mt-1 text-sm text-red-600 dark:text-red-400' data-testid='profile-username-error'>
                    {errors.username}
                  </p>
                )}
              </div>
              ...
              <div data-testid='profile-email-field'>
                <label htmlFor='email' className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2' data-testid='profile-email-label'>
                  Email Address *
                </label>
                <input
                  id='email'
                  ...
                  onChange={handleChange}
                  disabled={!isEditing}
                  className={`input ${!isEditing ? 'bg-gray-50 dark:bg-gray-800' : errors.email ? 'border-red-500 focus:ring-red-500' : ''}`}
                  placeholder='Email address'
                  data-testid='profile-email-input'
                />
                {errors.email && (
                  <p className='mt-1 text-sm text-red-600 dark:text-red-400' data-testid='profile-email-error'>
                    {errors.email}
                  </p>
                )}
              </div>
                    ...
                    >
                      {showPassword ? (
                        <EyeOff
                          ...
                          data-testid='profile-password-hide-icon'
                        />
                      ) : (
                        <Eye
                          className='h-5 w-5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'
                          data-testid='profile-password-show-icon'
                        />
                      )}
                    </button>
                  </div>
                  {errors.password && (
                    <p className='mt-1 text-sm text-red-600 dark:text-red-400' data-testid='profile-password-error'>
                      {errors.password}
                    </p>
                  )}
                </div>
              ...
              {isEditing && (
                <div className='flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700' data-testid='profile-form-actions'>
                  <button type='button' onClick={handleCancel} className='btn btn-outline' disabled={loading} data-testid='profile-cancel-btn'>
